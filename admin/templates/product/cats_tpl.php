...
<script language="javascript" type="text/javascript">
	$(document).ready(function() {
		...
		$("#xoahet").click(function(){
			...
			if (hoi==true) document.location = "index.php?com=product&act=delete_cat&type=<?=$type?>&listid=" + listid;
		});
	});
	...
	{				
		...
		window.location ="index.php?com=product&act=man_cat&type=<?=$type?>&id_danhmuc="+a.value;	
		return true;
	}
	...
	{				
		...
		window.location ="index.php?com=product&act=man_cat&type=<?=$type?>&id_danhmuc="+a.value+"&id_list="+b.value;	
		return true;
	}
	...
	{	
		...
		window.location ="index.php?com=product&act=man_cat&type=<?=$type?>&key="+a;	
		return true;
	}	
</script>
<?php
...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=product&act=man_cat<?php if($type!='') echo'&type='. $type;?>"><span>Danh mục cấp 3</span></a></li>
        	<?php if($_GET['key']!=''){ ?>
...
<div class="control_frm" style="margin-top:0;">
  	<div style="float:left;">
    	<input type="button" class="blueB" value="Thêm" onclick="location.href='index.php?com=product&act=add_cat&type=<?=$type?>'" />
...
<div class="widget">
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
   ...
   <tbody>
   <form name="frm" method="post" action="index.php?com=product&act=savestt_danhmuc<?php if($p!='') echo'&p='.$p;?>" enctype="multipart/form-data" class="nhaplieu">
   ...
   <tr>
        ...
        <td align="center">
         ...
         <td align="center">
   		...
   		<td class="title_name_data">
            <a href="index.php?com=product&act=edit_cat&id=<?=$items[$i]['id']?>&id_danhmuc=<?=$items[$i]['id_danhmuc']?>&id_list=<?=$items[$i]['id_list']?><?php if($type!='') echo'&type='.$type;?><?php if($p!='') echo'&p='.$p;?>" class="tipS SC_bold"><?=$items[$i]['ten']?></a>
        </td>
        <td align="center">
        ...
        <td class="actBtns">
            <a href="index.php?com=product&act=edit_cat&id=<?=$items[$i]['id']?>&id_danhmuc=<?=$items[$i]['id_danhmuc']?>&id_list=<?=$items[$i]['id_list']?><?php if($type!='') echo'&type='.$type;?><?php if($p!='') echo'&p='.$p;?>" title="" class="smallButton tipS" original-title="Sửa sản phẩm"><img src="./images/icons/dark/pencil.png" alt=""></a>

            <a href="index.php?com=product&act=delete_cat&id=<?=$items[$i]['id']?><?php if($type!='') echo'&type='.$type;?><?php if($p!='') echo'&p='.$p;?>" onClick="if(!confirm('Xác nhận xóa: <?=$items[$i]['ten']?>')) return false;" title="" class="smallButton tipS" original-title="Xóa sản phẩm"><img src="./images/icons/dark/close.png" alt=""></a>
        </td>
   </tr>
