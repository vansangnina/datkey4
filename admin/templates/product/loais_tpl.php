...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=product&act=man_item<?php if($type!='') echo'&type='. $type;?>"><span>Danh mục cấp 4</span></a></li>
        	<?php if($_GET['key']!=''){ ?>
...
<div class="widget">
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
   ...
   <tbody>
   <form name="frm" method="post" action="index.php?com=product&act=savestt_item<?php if($type!='') echo'&type='. $type;?><?php if($p!='') echo'&p='.$p;?>" enctype="multipart/form-data" class="nhaplieu">
   ...
   <tr>
        ...
        <td align="center">
   		...
   		<td class="title_name_data">
            <a href="index.php?com=product&act=edit_item&id=<?=$items[$i]['id']?>&id_danhmuc=<?=$items[$i]['id_danhmuc']?>&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?><?php if($type!='') echo'&type='. $type;?><?php if($p!='') echo'&p='.$p;?>" class="tipS SC_bold"><?=$items[$i]['ten']?></a>
        </td>
        ...
        <td class="actBtns">
            <a href="index.php?com=product&act=edit_item&id=<?=$items[$i]['id']?>&id_danhmuc=<?=$items[$i]['id_danhmuc']?>&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?><?php if($type!='') echo'&type='. $type;?><?php if($p!='') echo'&p='.$p;?>" title="" class="smallButton tipS" original-title="Sửa sản phẩm"><img src="./images/icons/dark/pencil.png" alt=""></a>

            <a href="index.php?com=product&act=delete_item&id=<?=$items[$i]['id']?><?php if($type!='') echo'&type='. $type;?><?php if($p!='') echo'&p='.$p;?>" onClick="if(!confirm('Xác nhận xóa: <?=$items[$i]['ten']?>')) return false;" title="" class="smallButton tipS" original-title="Xóa sản phẩm"><img src="./images/icons/dark/close.png" alt=""></a>
        </td>
   </tr>
