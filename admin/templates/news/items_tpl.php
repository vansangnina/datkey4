...
<script language="javascript" type="text/javascript">
	$(document).ready(function() {
		...
		$("#xoahet").click(function(){
			...
			if (hoi==true) document.location = "index.php?com=news&act=delete&type=<?=$type?>&listid=" + listid;
		});
	});
	...
	{				
		...
		window.location ="index.php?com=news&act=man&type=<?=$type?>&id_danhmuc="+a.value;	
		return true;
	}
	...
	{				
		...
		window.location ="index.php?com=news&act=man&type=<?=$type?>&id_danhmuc="+a.value+"&id_list="+b.value;	
		return true;
	}
	...
	{				
		...
		window.location ="index.php?com=news&act=man&type=<?=$type?>&id_danhmuc="+a.value+"&id_list="+b.value+"&id_cat="+c.value;	
		return true;
	}
	...
	{				
		...
		window.location ="index.php?com=news&act=man&type=<?=$type?>&id_danhmuc="+a.value+"&id_list="+b.value+"&id_cat="+c.value+"&id_item="+d.value;	
		return true;
	}
	...
	{	
		var a = $('input.key').val();	
		if(a=='Tên...') a='';		
		window.location ="index.php?com=news&act=man&type=<?=$type?>&key="+a;	
		return true;
	}	
</script>
...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=news&act=man&type=<?=$type?>"><span>Quản lý <?=$title_main ?></span></a></li>
        	<?php if($_GET['key']!=''){ ?>
...
<div class="control_frm" style="margin-top:0;">
  	<div style="float:left;">
    	<input type="button" class="blueB" value="Thêm" onclick="location.href='index.php?com=news&act=add<?php if($type!='') echo'&type='. $type;?>'" />
    ...
    <tbody>
       ...
       <?php if($_GET['type']=='nangluongmattroi'||$_GET['type']=='giaiphap'||$_GET['type']=='duan'||$_GET['type']=='tintuc') { ?>
        ...
        <td class="title_name_data">
            <a href="index.php?com=news&act=edit&id_danhmuc=<?=$items[$i]['id_danhmuc']?>&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&type=<?=$type?>&p=<?=$p?>&id=<?=$items[$i]['id']?>" class="tipS SC_bold"><?=$items[$i]['ten']?></a>
        </td>
        <td align="center">
        ...
        <td class="actBtns">
            <a href="index.php?com=news&act=edit&id_danhmuc=<?=$items[$i]['id_danhmuc']?>&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id_item=<?=$items[$i]['id_item']?>&type=<?=$type?>&p=<?=$p?>&id=<?=$items[$i]['id']?>" title="" class="smallButton tipS" original-title="Sửa sản phẩm"><img src="./images/icons/dark/pencil.png" alt=""></a>

            <a href="index.php?com=news&act=delete&id=<?=$items[$i]['id']?>&type=<?=$type?>&p=<?=$p?>" onClick="if(!confirm('Xác nhận xóa')) return false;" title="" class="smallButton tipS" original-title="Xóa tin"><img src="./images/icons/dark/close.png" alt=""></a>
        </td>
          </tr>
         <?php } ?>
    </tbody>
