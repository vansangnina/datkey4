...
<script language="javascript" type="text/javascript">
	...
	$("#xoahet").click(function(){
		...
		if (hoi==true) document.location = "index.php?com=slider&act=delete_photo&listid="+listid+"&id_slider=<?=$_REQUEST['id_slider']?>&type=<?=$type?>";
	});
	});
	...
	{	
		var a = $('input.key').val();
		if(a=='Tên...') a='';			
		window.location ="index.php?com=slider&act=man_photo&key="+a+"&id_slider=<?=$_REQUEST['id_slider']?>&type=<?=$type?>";	
		return true;
	}
</script>
...
<div class="widget">
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
    ...
    <tbody>
    <form name="f" id="f" method="post"  action="index.php?com=slider&act=savestt_photo<?php if($_REQUEST['id_slider']!='') echo'&id_slider='.$_REQUEST['id_slider'];?><?php if($type!='') echo'&type='.$type;?><?php if($p!='') echo'&p='.$p;?>">
    <?php for($i=0, $count=count($items); $i<$count; $i++){?>
       ...
       <td>
        ...
        <td class="actBtns">
            <a href="index.php?com=slider&act=edit_photo&id=<?=$items[$i]['id']?><?php if($_REQUEST['id_slider']!='') echo'&id_slider='. $_REQUEST['id_slider'];?><?php if($type!='') echo'&type='. $type;?><?php if($p!='') echo'&p='.$p;?>" title="" class="smallButton tipS" original-title="Sửa hình ảnh"><img src="./images/icons/dark/pencil.png" alt=""></a>
            <a href="index.php?com=slider&act=delete_photo&id=<?=$items[$i]['id']?><?php if($_REQUEST['id_slider']!='') echo'&id_slider='. $_REQUEST['id_slider'];?><?php if($type!='') echo'&type='. $type;?><?php if($p!='') echo'&p='.$p;?>" onClick="if(!confirm('Xác nhận xóa')) return false;" title="" class="smallButton tipS" original-title="Xóa hình ảnh"><img src="./images/icons/dark/close.png" alt=""></a>        </td>
      </tr>
         <?php } ?>
    </form>
    </tbody>
